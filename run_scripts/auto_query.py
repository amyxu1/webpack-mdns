...
def send_query(webpack_name, nic, ip):
    cl_args = ['/root/webpack-mdns/run_scripts/mdns-controller', '--net', nic, '--ip', ip, '--query', webpack_name]
...
def main(argv):
    if len(argv) < 3:
        print('usage: python3 auto_query.py [# of queries to send] [nic] [wpack name(s)]')
        sys.exit(2)

    num_queries = int(argv[1])
    nic = argv[2]
    ip = ni.ifaddresses(nic)[ni.AF_INET][0]['addr']
    webpack_names = argv[3:]
    ...
    for i in range(1, num_queries+1):
        ...
        runtime = send_query(webpack_names[i-1], nic, ip)#send_query(webpack_names[(num_packs - 1) % i])
